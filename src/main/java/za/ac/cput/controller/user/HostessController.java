...
public class HostessController {
    ...
    @Autowired
    public HostessController (HostessServiceImpl service) {this.service = service;}
    //
    @PostMapping("/save")
    public ResponseEntity<Hostess> save (@RequestBody Hostess hostess){
        log.info("Request to save plane: {}", hostess);
    ...
    @GetMapping("/read/{id}")
    public ResponseEntity <Hostess> read (@PathVariable String id)
    {
    ...
    public ResponseEntity <Hostess> delete (Hostess hostess){
        log.info("Request to delete plane: {}", hostess);
        this.service.delete(hostess);
        return ResponseEntity.ok(hostess);
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity <Hostess> deleteById(@PathVariable String id){
        log.info("Request to delete plane: {}", id);
        this.service.deleteById(id);
        return ResponseEntity.ok().build();
    }
    @PutMapping("/update/{id}")
    public ResponseEntity<Hostess> update(@RequestBody Hostess hostess, @PathVariable String id) {
        try {
            Hostess existPlane =service.read(id).orElse(null);
